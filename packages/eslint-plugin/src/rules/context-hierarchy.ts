...
const rule: Rule.RuleModule = {
  meta: {
    ...
    messages: {
      unknownType: 'Unknown @context type "{{type}}". Valid types: {{validTypes}}.',
      invalidSubtype:
    ...
    return {
      Program() {
        ...
        for (const tag of tags) {
          ...
          if (tag.subtype && !isValidSubtype(tag.type, tag.subtype)) {
            const validSubs = getValidSubtypes(tag.type);
            context.report({
              loc: { line: tag.line, column: tag.column },
